  ...
  if (req.method === "PUT") {
    ...
    if (verificationStatus === "VERIFIED" && broker.verificationStatus !== "VERIFIED") {
      await notifyUser({
        ...
        adminId: session.user.id,
        subject: "모기지 전문가 인증이 완료되었습니다 / You're verified",
        body:
          "모기지 전문가 인증이 완료되었습니다. 이제 상담 요청을 보고 응답할 수 있습니다. / " +
      ...
      await notifyUser({
        userId: broker.user.id,
        adminId: session.user.id,
        subject: "모기지 전문가 인증이 거절되었습니다 / Verification not approved",
        body:
          `모기지 전문가 인증이 거절되었습니다.${reasonKo} 문의는 고객센터로 연락해 주세요. / ` +
          `Your broker verification was not approved.${reasonEn} Contact support to review your profile.`,
        email: {
          subjectKo: "모기지 전문가 인증이 거절되었습니다",
          subjectEn: "Your verification was not approved",
          bodyKo: `모기지 전문가 인증이 거절되었습니다.${reasonKo}`,
