...
/**
 * Unified admin chrome. Replaces AdminLayout + AdminSidebarNav + the marketing
 * `Layout` wrapper previously used on admin pages. Provides:
 *   - 72px left rail (5 items, urgent-count dots, avatar)
 *   - Full-width top bar with ⌘K trigger + status dot + admin badge
 *   - Scrollable content slot
 *   - Badge-count fetcher hitting /api/admin/stats
 *   - ⌘K / Ctrl+K keyboard shortcut that opens the command palette
 *
 * The command palette itself is a lazy-loaded sibling — the shell just owns
 * the open/close state and renders it when open.
 */
...
interface NavItem {
  ...
  labelKey: string;
  fallback: string;
  /** count key from /api/admin/stats; undefined items don't show a dot */
...
interface BadgeCounts {
  ...
  openReports: number;
  /** derived: pendingVerifications + pendingRequests + openReports */
  inbox: number;
}
...
export interface AdminShellProps {
  ...
  children: ReactNode;
  /**
   * Page title for <title>. Rendered as-is; callers can use a t() key.
   * Falls back to "Admin — mortly".
   */
  pageTitle?: string;
}
...
export default function AdminShell({ active, children, pageTitle }: AdminShellProps) {
  ...
  // Periodic badge fetch (same endpoint + cadence as the old sidebar)
  ...
  useEffect(() => {
    if (paletteOpen && !PaletteComp) {
      // Dynamic import avoids bundling the palette into every page load.
    ...
    <CommandPaletteContext.Provider value={{ open: paletteOpen, setOpen: setPaletteOpen }}>
      ...
      <div className="flex h-screen w-full overflow-hidden bg-cream-100 text-forest-800">
        ...
        <aside className="w-[72px] shrink-0 flex flex-col items-center py-4 bg-forest-800 text-cream-100">
          ...
          <div className="mt-1 font-mono text-[8px] tracking-[0.2em] text-cream-100/40">
            ADMIN
          </div>
          ...
          {/* Admin avatar → account menu (sign out) */}
        ...
        {/* Command palette (lazy-loaded on first open) */}
...
// Clickable avatar at the bottom of the rail. Opens a tiny popover with
// account-level actions (currently just sign out). The popover sits to
// the right of the avatar so it doesn't get clipped by the rail edge.
...
function AdminAvatarMenu({ adminName }: { adminName: string }) {
  ...
  const handleSignOut = async () => {
    setOpen(false);
    // callbackUrl sends the user to /login after the sign-out completes.
