...
// The brokers who responded to a single request, shown as comparable profile
// cards sorted by speed or experience. Lives on the request hub
// (/borrower/request/[id]); previously a standalone /borrower/brokers/[id] page.
export interface ConversationBroker {
  id: string;
  createdAt: string;
  status: string;
...
function sortConversations(
  ...
  switch (sort) {
    ...
    case "most_experienced":
      sorted.sort(
        (a, b) => (b.broker.yearsExperience ?? 0) - (a.broker.yearsExperience ?? 0),
      );
      break;
  }
  return sorted;
...
export default function BrokerResponses({
  conversations,
}: {
  conversations: ConversationBroker[];
}) {
  ...
  if (conversations.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-body-sm text-sage-400">{t("brokerIntros.noIntrosDesc")}</p>
      </div>
    );
  }
  ...
  return (
    <div>
      {/* Disclaimer — broker-provided info, verify independently */}
            ...
            >
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-body-sm">{broker.brokerageName}</p>
              ...
              {broker.specialties && (
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  <span className="text-xs font-body font-medium text-sage-500">
                    {t("brokerComparison.specialties")}:
                  </span>
              ...
              <div className="mt-6 flex items-center gap-3 pt-5 border-t divider">
                <button
                  onClick={() =>
                    router.push(`/borrower/messages?id=${conv.id}`, undefined, {
